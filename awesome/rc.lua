...
memwidget:set_background_color("#393B3F")
...
beautiful.init("/home/brmiller/.config/awesome/themes/default/theme.lua")
...
tags = {}
for s = 1, screen.count() do
    -- Each screen has its own tag table. default to layout[2]: fair
...
clientkeys = awful.util.table.join(
    ...
    awful.key({ modkey, "Control" }, "h",     function (c)
                awful.client.moveresize(0, 0, -c.size_hints.width_inc, 0)
        end),
...
function run_once(prg)
    ...
    awful.util.spawn_with_shell("pgrep -u $USER -f -x '" .. prg .. "' || (exec " .. prg .. ")")
end

run_once("nm-applet")
awful.util.spawn_with_shell("xrdb -load -override ~/.Xdefaults")
awful.util.spawn_with_shell("if [ `lsusb | grep -c 'Logitech, Inc. MX Revolution/G7 Cordless Mouse'` -eq 1 ]; then xmodmap -e 'pointer = 1 8 3 4 5 6 7 2 9'; else xmodmap -e 'pointer = 1 2 3 4 5 6 7 8 9'; fi")
