...
function displayMonth(month, year, weekStart)
	...
	-- print the days of the week starting at Sunday (2006 starts on Sunday)
	for x = 0,6 do
		lines = lines .. os.date("%a ",
			os.time{ year=2006, month=1, day = x+wkSt})
	end
...
function switchNaughtyMonth(switchMonths)
	...
	calendar[3] = naughty.notify({
		text = string.format('<span font_desc="%s">%s</span>', "monospace", 
			displayMonth(calendar[1], calendar[2], 1)),
		timeout = 0,
		hover_timeout = 0.5,
